...
class TimeClock {
    ...
    update(elapsed) {
        if(this._isStarted) {
            const time = new Date();
            const pad = this.pad;
            const str = `${pad(time.getHours(), 2)}:${pad(time.getMinutes(), 2)}:${pad(time.getSeconds(), 2)}`;
            $("#time-value").text(str);
        }
    ...
    /**
     * @link https://stackoverflow.com/a/10073788
     * @author Pointy
...
class RecordButton extends Button {
    ...
    prepare() {
        ...
        $(this._element).html(`<i class="flag outline icon"></i>기록하기`);
    }
    ...
    onClick() {
        ...
        $(this._element).html(`<i class="stop icon"></i>중지`);
